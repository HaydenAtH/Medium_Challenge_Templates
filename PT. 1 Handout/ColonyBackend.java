    ...
    public static void main(String[] args) throws Exception {
        ...
        for (int x = 0; x < size; x++){
            for (int y = 0; y < size; y++){
                ...
                t[j] = new Tile(Math.random() > 0.5 ? (int) (Math.random() * 8) : 0, isColony);
        ...
        if (colonyCount == 0) {
            ...
            t[index] = new Tile(Math.random() > 0.5 ? (int) (Math.random() * 8) : 0, true);
            ...
            for (int i = 0; i < 80; i++){
                ...
                for (Tile c : t){
                    if (c.isColony() && c.getResources() < 3){
                        throw new Exception("Completed " + i + " ticks before a Colony went below 3 resources! Check your solution and run again");
                    }else if (c.isColony()){
                        if (a == -1) {
                            a = c.getResources();
                        }else{
                ...
                for (int x = 0; x < size; x++){
                    for (int y = 0; y < size; y++){
                        ...
                        t[j] = new Tile(Math.random() > 0.5 && !isColony ? t[j].getResources() + (int) (Math.random() * 4) : isColony ? t[j].getResources() - (int) (Math.random() * t[j].getResources() - 1) : t[j].getResources(), isColony);
                        hashList.add(t[j].hashCode());
